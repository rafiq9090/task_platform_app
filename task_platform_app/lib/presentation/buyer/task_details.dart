...
class TaskDetailsScreen extends StatelessWidget {
        ...
        child: Column(
          ...
          children: [
            ...
            if (task.hoursSpent != null)
              _buildRow('Hours Logged', '${task.hoursSpent} hrs'),
            if (task.hoursSpent != null)
              _buildRow(
                'Total Amount',
                currencyFormat.format(task.totalAmount),
                isBold: true,
              ),
          ],
        ),
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Payment of ${format.format(task.totalAmount)} is required to unlock the solution.',
                ),
              ),
            ],
          ),
        ...
        ElevatedButton(
          onPressed: provider.isLoading
              ? null
              : () async {
                  final success = await provider.payForTask(
                    task.id,
                    task.projectId,
                  );
                  if (success && context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Payment Successful! Solution Unlocked.'),
                      ),
                    );
  ...
  Widget _buildDownloadSection(BuildContext context, TaskProvider provider) {
    Future<void> _handleFileOpening(String path) async {
      ...
      children: [
        ...
        OutlinedButton.icon(
          onPressed: provider.isLoading
              ? null
              : () async {
                  final path = await provider.downloadSolution(
                    task.id,
                    task.title,
                  );
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        behavior: SnackBarBehavior
                            .floating, // Recommended for modern feel
                        content: Text(
                          'Downloaded: ${path.split('/').last}',
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                        ),
                        action: SnackBarAction(
                          label: 'Open',
                          onPressed: () => _handleFileOpening(
                            path,
                          ), // Fixed the double underscore typo
                        ),
                      ),
                    );
                  }
